...
Процедура ВыполнитьОбмен(Команда)
	...
	// +++ Вдовин П.Ю. 25.06.2026 Двухфазный обмен XML: команда «Выполнить обмен» на форме приёмника
	
	Если Не ЗначениеЗаполнено(Объект.ПрофильПриемника) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбран профиль приёмника.'"));
		Возврат;
	КонецЕсли;
	
	РезультатСеанса = ВыполнитьСеансОбменаПоПрофилюПриемникаНаСервере();
...
&НаСервере
Функция ВыполнитьСеансОбменаПоПрофилюПриемникаНаСервере()
	
	// +++ Вдовин П.Ю. 25.06.2026 Двухфазный обмен XML: команда «Выполнить обмен» на форме приёмника
	
	Возврат рг_УниверсальныйОбменXMLСервер.ВыполнитьСеансОбменаПоПрофилюПриемника(Объект.ПрофильПриемника);
